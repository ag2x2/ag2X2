...
class BaseTask():
    ...
    def update_smo_metric(self):
        if len(self.gripper_pos_history) < 2:
            return
        gripper_positions = np.array(self.gripper_pos_history)
        another_gripper_positions = np.array(self.another_gripper_pos_history)
        vels = np.diff(gripper_positions, axis=0)
        another_vels = np.diff(another_gripper_positions, axis=0)
        accs = np.diff(vels, axis=0)
        another_accs = np.diff(another_vels, axis=0)
        acc_mag = np.linalg.norm(accs, axis=1)
        another_acc_mag = np.linalg.norm(another_accs, axis=1)
        current_smo = np.sqrt(np.sum(acc_mag**2)+np.sum(another_acc_mag**2))
        self.smo_list.append(current_smo)

    def get_avg_smo(self):
        if len(self.smo_list) > 0:
            avg_smo = np.mean(self.smo_list)
            return avg_smo
        else:
            return 0
